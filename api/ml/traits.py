...
The real trait table is a Module-3 objective (USDA PLANTS + Perenual + Trefle
+ Wikidata, joined on a WFO-normalized binomial). This is a hand-seeded subset
so the arrangement engine has something real to reason over today.
"""
...
_ROOT = Path(__file__).resolve().parents[2]  # api/ml -> project root
