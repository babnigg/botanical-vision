...
can pull and compare it. First time only: ``huggingface-cli login``.
    ...
    python -m share.publish --checkpoint checkpoints/resnet50_improved_best.pt \
...
from . import hub
from .constants import ARCH, IMG_SIZE, MEAN, ROOT, STD
...
def publish(checkpoint: str, name: str, author: str | None = None, notes: str = "") -> str:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        meta = {
            "author": author, "name": name, "arch": ARCH, "notes": notes,
    ...
    print("teammates can now run `python -m share.leaderboard` to compare.")
    return base
